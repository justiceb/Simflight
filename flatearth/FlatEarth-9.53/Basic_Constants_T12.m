...
alpha_0 = -4*pi/180;                % Airfoil zero-lift AOA [rad]
altitude= 300;                % Trim altitude [ft]
disp(['Trim altitude= ',num2str(altitude),' ft'])
AR_h = 2.67;                        % Aspect ratio of the horizontal tail
AR_w = 6.86;              % Aspect ratio of the wing
b_f = 13/12;                 % Span of the flap [ft] (Alieron total span)****
b_h = 12/12;                   % Span of the horizontal tail [ft]
b_h_oe = 12/12;                % Elevator outboard position [ft]
b_h_ie = 0;                  % Elevator inboard position [ft]
b_w = 68/12;                  % Span of the wing [ft]
b_v = 4.5/12;                  % Vertical tail span measured from fuselage centerline[ft]
b_v_or = 12/12;                  % Outboard position of rudder [ft]
b_v_ir = 0;                     % Inboard position of rudder [ft]
c_a = 3/12;                   % Chord of aileron [ft]
C_bar_D_o = 0.015;       % Parasite drag
Cd_0 = 0.016;               % Drag coefficient at zero lift (parasite drag)
c_e = 2/12;                         % Elevator chord [ft]
cf = 0/12;                      % Chord of the wing flap [ft]
c_h = 3/12;                  % Mean aerodynamic chord of the horizontal tail [ft]
Cl_alpha_h = 6.8;                % 2-D Lift curve slope of horizontal tail
Cl_alpha_v = 6.8;              % 2-D Lift curve slope of vertical tail
Cl_alpha = 5.73;             % 2-D lift curve slope of whole aircraft
Cl_alpha_w = 5.73;           % 2-D lift curve slope of wing
...
c_r =  5/12;                 % MEAN Chord of the rudder [ft]
c_w = 10.5/12;                 % Mean aerodynamic chord of the wing [ft]
c_v = 5/12;                   % Mean aerodynamic chord of the vertical tail [ft]
D_p = 10/12;          % Diameter of propeller [ft]
d = 8/12;                                % Average diameter of the fuselage [ft]
...
h1_fuse =5/12;               % Height of the fuselage at 1/4 of the its length 
h2_fuse = 5/12;              % Height of the fuselage at 3/4 of the its length 
h_h = 0/12;                   % Height from chord plane of wing to chord plane of
%                                                horizontal tail [ft] - Fig 3.7, Ref. 2
hmax_fuse = 5/12;             % Maximum height of the fuselage [ft]  
...
l_f = 18.5/12;                   % Horizontal length of fuselage [ft]  
l_v = 20/12;                   % Horizontal distance from aircraft arbitrary reference point to vertical tail AC [ft]
...
% Trim Airspeed
u = 40; % ft/sec
S_b_s = 92.5/144;            % Body side area [ft^2]
S_h = 50/144;                    % Area of horizontal tail [ft^2]
S_h_slip = 30/144;                % Area of horizontal tail that is covered in 
%                                                prop-wash [ft^2] - See Fig.(8.64) - Ref.(3) ***EST
S_o = 8*5/144;                    % Fuselage x-sectional area at Xo [ft^2] - 
%                                                See Fig.(7.2) - Ref.(2)
%                                                 Xo is determined by plugging X1/l_b into: 
%                                                0.378 + 0.527 * (X1/l_b) = (Xo/l_b)
S_w = 714/144;          % Surface area of wing [ft^2]
S_v = 35/144;               % Surface area of vertical tail [ft^2]
...
two_r_one = 1/12;             % Fuselage depth in region of vertical tail [ft] Ref.(2),Figure 7.5

U = u/1.687808; % knots                % Free Stream Velocity (Trim velocity) [KNOTS true]
disp(['Trim airspeed= ',num2str(U),' knots'])
W = 6;           % Weight of Airplane [lbf]
wingloc = 1;                % If the aircraft is a highwing: (wingloc=1), low-wing:(wingloc=0) 
wmax_fuse = 8/12;   % Maximum fuselage width [ft]
X1 = 18.5/12;                           % Distance from the front of the fuselage where the 
%                                                x-sectional area decrease (dS_x/dx) 
%                                                is greatest (most negative) [ft] - Ref.(2),Fig. 7.2  
x_m = 8/12;                    % Distance from nose of aircraft to arbitrary reference point [ft]
...
Xach = 20/12;            % Distance from the leading edge of the wing mean aerodynamic chord
%                       to the aerodynamic center of the horizontal tail (positive aftward) [ft]
Xacwb = .35*c_w;     % Distance from the leading edge of the wing mean aerodynamic chord
...
%                                                Measured as positive aft, starting from the leading edge of the mean aero. chord. [ft]
Xref = 0/12;      % Distance from the leading edge of the wing mean aerodynamic chord
...
Z_h = -(2.5)/12;                        % Negative of the VERTICAL distance from the fuselage 
...
Z_v = 2.5/12;                            % Vertical distance from the aircraft arbirary reference point to the vertical 
%                                                tail aero center (positive up) - Ref.(2), Fig. 7.18
Z_w = 2.5/12;                               % This is the vertical distance from the wing root c/4 [ft]
%                                                to the fuselage centerline, 
%                                                positive downward - Ref.(2), Equ(7.5)
Z_w1 = 2.5/12;                         % Distance from body centerline to c/4 of wing root 
